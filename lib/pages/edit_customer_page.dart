...
class EditCustomerPage extends StatefulWidget {
  ...
  const EditCustomerPage({super.key, required this.customerId, required this.customerData});
...
class _EditCustomerPageState extends State<EditCustomerPage> {
  ...
  void initState() {
    ...
    _descCtrl = TextEditingController(text: widget.customerData["description"] ?? "");
  }
  ...
  Future<void> _updateCustomer() async {
    ...
    await FirebaseFirestore.instance.collection("customers").doc(widget.customerId).update({
      ...
      "updatedAt": DateTime.now().toIso8601String(), // Update time moves customer to top
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Edit Profile"), backgroundColor: Colors.blue[800], foregroundColor: Colors.white),
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            TextField(controller: _phoneCtrl, decoration: const InputDecoration(labelText: "Phone", border: OutlineInputBorder(), prefixIcon: Icon(Icons.phone))),
            const SizedBox(height: 16),
            TextField(controller: _descCtrl, maxLines: 3, decoration: const InputDecoration(labelText: "Description", border: OutlineInputBorder(), prefixIcon: Icon(Icons.description))),
